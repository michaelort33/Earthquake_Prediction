...
# -*- coding: utf-8 -*-
"""
Created on Fri Jan 25 15:01:29 2019
...
def add_trend_feature_abs_t(arr, abs_values=True):
    idx = np.array(range(len(arr)))
    if abs_values:
        arr = np.abs(arr)
    lr = LinearRegression()
...
def sin_coef(data,z):
    ...
    return [est_amp, est_freq, est_phase, est_mean][z]
...
for segment in tqdm(range(segments)):
    ...
    X_tr.loc[segment, 'sine_0'] = sin_coef(x,0)
    X_tr.loc[segment, 'sine_1'] = sin_coef(x,1)
    X_tr.loc[segment, 'sine_2'] = sin_coef(x,2)
    X_tr.loc[segment, 'sine_3'] = sin_coef(x,3)
    
    X_tr.loc[segment, 'sine_last_0'] = sin_coef(x[-10000:],0)
    X_tr.loc[segment, 'sine_last_1'] = sin_coef(x[-10000:],1)
    
    
    
    
    for windows in [50, 100, 1000]:
        ...
        X_tr.loc[segment, 'sin_0_roll_mean_' + str(windows)] = sin_coef(x_roll_mean,0)
        X_tr.loc[segment, 'sin_1_roll_mean_' + str(windows)] = sin_coef(x_roll_mean,1)
        
        X_tr.loc[segment, 'sin_0_roll_std_' + str(windows)] = sin_coef(x_roll_std,0)
        X_tr.loc[segment, 'sin_1_roll_std_' + str(windows)] = sin_coef(x_roll_std,1)
        
    new_rows = 50
    new_segments = int(np.floor(len(x)/50))
    my_sin_coef = []
    
    for mini_windows in range(0,new_segments,50):
        new_x = x[new_rows*mini_windows:new_rows*mini_windows+new_rows]
        coef = sin_coef(new_x,0)
        my_sin_coef.append(coef)
        
    X_tr.loc[segment, 'sin_coef_min'] = min(my_sin_coef)
    X_tr.loc[segment, 'sin_coef_max'] = max(my_sin_coef)
    X_tr.loc[segment, 'sin_coef_mean'] = sum(my_sin_coef)/len(my_sin_coef)
    X_tr.loc[segment, 'sin_coef_std'] = st.stdev(my_sin_coef)
...
for i, seg_id in enumerate(tqdm(X_test.index)):
    ...
    X_test.loc[seg_id, 'sine_0'] = sin_coef(x,0)
    X_test.loc[seg_id, 'sine_1'] = sin_coef(x,1)
    X_test.loc[seg_id, 'sine_2'] = sin_coef(x,2)
    X_test.loc[seg_id, 'sine_2'] = sin_coef(x,3)
    
    X_test.loc[seg_id, 'sine_last_0'] = sin_coef(x[-10000:],0)
    X_test.loc[seg_id, 'sine_last_1'] = sin_coef(x[-10000:],1)
    ...
    for windows in [50, 100, 1000]:
        ...
        X_test.loc[seg_id, 'sin_0_roll_mean_' + str(windows)] = sin_coef(x_roll_mean,0)
        X_test.loc[seg_id, 'sin_1_roll_mean_' + str(windows)] = sin_coef(x_roll_mean,1)
        
        X_test.loc[seg_id, 'sin_0_roll_std_' + str(windows)] = sin_coef(x_roll_std,0)
        X_test.loc[seg_id, 'sin_1_roll_std_' + str(windows)] = sin_coef(x_roll_std,1)
    ...
    X_test.loc[seg_id, 'sin_coef_min'] = min(my_sin_coef)
    X_test.loc[seg_id, 'sin_coef_max'] = max(my_sin_coef)
    X_test.loc[seg_id, 'sin_coef_mean'] = sum(my_sin_coef)/len(my_sin_coef)
    X_test.loc[seg_id, 'sin_coef_std'] = st.stdev(my_sin_coef)
...
X_train_scaled.to_pickle('../input/X_train_scaled.pkl')
X_tr.to_pickle('../input/X_tr.pkl')

X_test_scaled.to_pickle('../input/X_test_scaled.pkl')
X_test.to_pickle('../input/X_test.pkl')

y_tr.to_pickle('../input/y_tr.pkl')
submission.to_pickle('../input/submission.pkl')
